...
import static fr.gardahaut.test1.SetCommand.*;
...
public class CutCommand extends AbstractPlayerCommand {
    ...
    public CutCommand(@NonNullDecl String name, @NonNullDecl String description, LandmarksStore landmarksStore, ClipboardStore clipboardStore) {
        ...
        for (int x = minX; x <= maxX; x++) {
            for (int y = minY; y <= maxY; y++) {
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    if (blockType != null) {
                        Logger.getLogger("AAA").log(Level.INFO, blockType.getId());
